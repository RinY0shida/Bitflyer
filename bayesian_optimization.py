...
def miniutu_response():
    ...
    behavior_average = [0] * pull_amount

    prev_response = requests.get("https://min-api.cryptocompare.com/data/v2/histominute?fsym=BTC&tsym=JPY&limit=1439")  #1日分の分足を取得
    ...
    for i in stock_data:
        ...
        behavior_average[count] = np.sum(prev_close) / (count + 1)
    ...
    plt.plot(prev_close , color = 'b')
    plt.plot(behavior_average ,  color = 'r')
    plt.show()
...
def day_response():
    ...
    prev_response = requests.get("https://min-api.cryptocompare.com/data/v2/histoday?fsym=BTC&tsym=JPY&limit=300")  #1日分の分足を取得
